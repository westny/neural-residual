...
def init_strategy(seq_module: Sequential,
                  ...
                  n_non_states: int = 0,
                  eps: float = -1e-2,
                  bias_init: float = 1e-4) -> torch.Tensor:
    """
    ...
    n_non_states : int, optional
        The number of states that are not dynamic (default is 0).
    eps : float, optional
    ...
    past = n_states + n_inputs + n_non_states
    nxt = n_hidden
    ...
    for module in seq_module:
        if isinstance(module, Linear):
            ...
            if ji == 0:
                ...
                cn[n_states + n_non_states:n_states + n_inputs + n_non_states, :n_states] = input_init
